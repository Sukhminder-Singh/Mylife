...
import {setLoader, loginUser} from '../actions';
import MyStatusBar from '../components/MySatusBar';
import WaveSpinner from '../components/Spinner';
...
function LoginScreen(props) {
  ...
  const loginHandler = async () => {
    ...
    setTimeout(() => {
      if (data.email && data.password) {
        ...
        props.loginUser({email, password}).then(result => {
  ...
  return (
    <KeyboardAvoidingView style={styles.container}>
      ...
      <Text style={styles.text1}>{Texts.letsgo}</Text>
      <View style={styles.logincontainer}>
        ...
        <TouchableOpacity onPress={loginHandler} style={styles.buttonStyle}>
          <Text style={styles.text2}>{Texts.loginText}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => props.navigation.navigate('ForgotPassword')}>
          <Text style={styles.forgot}>{Texts.forgot}</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.newAccountcontainer}>
        <Text style={styles.text3}>{Texts.newAccount}</Text>
        <TouchableOpacity>
          <Text style={styles.forgot}>{Texts.createnew}</Text>
        </TouchableOpacity>
      </View>
      {props.isLoading ? <WaveSpinner /> : null}
      <DropdownAlert
        ref={ref => {
          if (ref) {
...
export default connect(mapStateToProps, {setLoader, loginUser})(LoginScreen);
